...
export default function CreateQuest({ onSuccess }: { onSuccess: () => void }) {
  ...
  const [duration, setDuration] = useState(86400); // Default 1 Hari
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-6 rounded-3xl shadow-lg">
      <div className="flex flex-col gap-4">
        <input value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Question" className="w-full p-3 bg-gray-50 dark:bg-gray-800 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500" />
        <div className="grid grid-cols-2 gap-3">
            <input value={opt1} onChange={(e) => setOpt1(e.target.value)} placeholder="Yes" className="p-3 bg-blue-50/50 border rounded-xl outline-none" />
            <input value={opt2} onChange={(e) => setOpt2(e.target.value)} placeholder="No" className="p-3 bg-pink-50/50 border rounded-xl outline-none" />
        </div>
        ...
        {canUsePaymaster && (
            <div className="flex justify-center mt-2">
                <div className={`flex items-center gap-2 px-5 py-2 rounded-full cursor-pointer transition-all border shadow-sm ${usePaymaster ? 'bg-blue-600 border-blue-600 text-white shadow-blue-500/20' : 'bg-gray-100 text-gray-500'}`} onClick={() => setUsePaymaster(!usePaymaster)}>
