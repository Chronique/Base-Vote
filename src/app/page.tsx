...
export default function Home() {
  ...
  // State untuk memaksa tampilan meme setelah pembuatan poll
  ...
  return (
    <main className="h-[100dvh] w-full bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100 font-sans flex flex-col fixed inset-0 overflow-hidden transition-colors duration-300">
      ...
      <nav className="flex-none w-full bg-white/80 dark:bg-gray-950/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 px-4 py-3 flex justify-between items-center z-50">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center rounded-full border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 shadow-sm overflow-hidden">
              <button onClick={handleShare} className={`flex items-center justify-center p-2 px-3 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 active:bg-blue-100 transition-colors ${!isAdded ? "border-r border-gray-200 dark:border-gray-700" : ""}`}>
                  <MdShare className="text-xl" />
              </button>
              {!isAdded && (
                  <button onClick={handleAddApp} className="flex items-center justify-center p-2 px-3 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 active:bg-blue-100 transition-colors">
          ...
          {farcasterUser ? (
            ...
            <div className="ml-1"><ConnectButton showBalance={false} accountStatus="avatar" chainStatus="none" /></div>
          )}
        </div>
      </nav>
      ...
      <div className="flex-1 relative w-full max-w-lg mx-auto overflow-hidden">
        <div className={`absolute inset-0 px-4 pt-4 pb-24 ${activeTab === 'profile' ? 'overflow-y-auto' : 'overflow-hidden'}`}>
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                {/* SETELAH BERHASIL: Aktifkan forceMeme dan pindah ke Feed */}
            ...
            <div className="flex flex-col h-full">
                <div className="flex-none relative w-full h-28 rounded-3xl overflow-hidden shadow-xl group bg-gradient-to-r from-blue-600 to-blue-400 dark:to-gray-800 mb-4 flex items-center px-6 gap-4">
                    ...
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className="relative z-20 flex-none w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-lg shadow-blue-900/20">
                        <div className="w-12 h-12 bg-blue-600 rounded-full flex items-center justify-center relative overflow-hidden"><div className="absolute top-2 w-8 h-8 border-t-4 border-white rounded-full"></div></div>
                    </motion.div>
                    <div className="relative z-10 flex flex-col justify-center overflow-hidden h-full py-2">
                        <motion.div initial={{ x: -100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ type: "spring", damping: 15, stiffness: 100, delay: 0.3 }}>
                ...
                <div className="flex-1 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-100 flex flex-col">
                    ...
                    <div className="flex-1 flex items-center justify-center -mt-4">
                        {/* TERUSKAN initialMeme ke QuestList */}
      ...
      <div className="flex-none w-full bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 flex justify-around py-3 z-50 pb-6 max-w-lg mx-auto shadow-[0_-4px_15px_-3px_rgba(0,0,0,0.1)]">
        <button onClick={() => { setForceMeme(false); setActiveTab("feed"); }} className={`flex flex-col items-center gap-1 w-1/3 transition-all active:scale-90 ${activeTab === "feed" ? "text-blue-600 dark:text-blue-400" : "text-gray-400 dark:text-gray-600"}`}><MdHomeFilled className="text-2xl" /><span className="text-[10px] font-bold">FEED</span></button>
        <button onClick={() => setActiveTab("create")} className={`flex flex-col items-center gap-1 w-1/3 transition-all active:scale-90 ${activeTab === "create" ? "text-blue-600 dark:text-blue-400" : "text-gray-400 dark:text-gray-600"}`}><MdAddCircle className="text-2xl" /><span className="text-[10px] font-bold">CREATE</span></button>
        <button onClick={() => setActiveTab("profile")} className={`flex flex-col items-center gap-1 w-1/3 transition-all active:scale-90 ${activeTab === "profile" ? "text-blue-600 dark:text-blue-400" : "text-gray-400 dark:text-gray-600"}`}><MdPerson className="text-2xl" /><span className="text-[10px] font-bold">PROFILE</span></button>
      </div>
    </main>
  );
}