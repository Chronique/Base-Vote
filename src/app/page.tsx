...
// Import SDK & Wagmi Hooks
import { sdk } from "@farcaster/miniapp-sdk"; 
import { useConnect, useAccount } from "wagmi"; // Tambah hooks ini
...
export default function Home() {
  ...
  // State untuk menyimpan data User Farcaster
  const [farcasterUser, setFarcasterUser] = useState<{ pfpUrl?: string; username?: string } | null>(null);

  // Hook Wagmi untuk Auto-Connect
  const { connect, connectors } = useConnect();
  const { isConnected } = useAccount();

  // Fungsi Inisialisasi (Auto Connect & Load Data)
  const initializeFarcaster = useCallback(async () => {
    try {
      // 1. Ambil Context dari Farcaster (Data User)
      ...
      // 2. Tampilkan Aplikasi (Hide Splash Screen)
      await sdk.actions.ready();

      // 3. AUTO CONNECT WALLET
      // Jika belum konek, cari konektor 'farcasterFrame' dan langsung konekkan
      if (!isConnected) {
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100 font-sans flex flex-col pb-24 transition-colors duration-300">
      ...
      <nav className="w-full bg-white/90 dark:bg-gray-950/90 backdrop-blur border-b border-gray-100 dark:border-gray-800 px-4 py-3 flex justify-between items-center sticky top-0 z-50 transition-colors">
        ...
        {/* BAGIAN KANAN: PROFIL USER (GANTI CONNECT BUTTON) */}
        <div>
          {farcasterUser ? (
            // TAMPILAN JIKA DI DALAM FARCASTER (PROFIL)
            ...
            // FALLBACK: JIKA DIBUKA DI BROWSER BIASA (TOMBOL LAMA)
            <div className="px-3 py-1.5 bg-blue-600 text-white text-xs font-bold rounded-lg shadow-sm">
               Login via Warpcast
            </div>
          )}
        </div>
